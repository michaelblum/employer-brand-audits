#!/usr/bin/env python3
"""
Brainstorming preview server.
Watches .brainstorm/content/ for HTML fragments and serves the newest one
wrapped in the frame template. Writes click events to .brainstorm/state/events.
"""
...
CONTENT_DIR = BASE / ".brainstorm" / "content"
STATE_DIR = BASE / ".brainstorm" / "state"
...
body { font-family: system-ui,-apple-system,BlinkMacSystemFont,sans-serif; background: var(--bg-primary); color: var(--text-primary); display: flex; flex-direction: column; line-height: 1.5; }
...
.card { background: var(--bg-secondary); border: 1px solid var(--border); border-radius: 12px; overflow: hidden; cursor: pointer; transition: all 0.15s ease; }
...
(function() {
  let lastMod = null;
  function poll() {
    fetch('/api/current-mtime').then(r=>r.json()).then(d=>{
      if (lastMod && d.mtime !== lastMod) window.location.reload();
      lastMod = d.mtime;
    }).catch(()=>{});
    setTimeout(poll, 800);
  }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!target) return;
    const event = { type:'click', text: target.textContent.trim(), choice: target.dataset.choice, id: target.id||null, timestamp: Date.now() };
    fetch('/api/event', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(event) });
    setTimeout(() => {
  ...
  window.toggleSelect = function(el) {
    ...
    el.classList[multi ? 'toggle' : 'add']('selected');
  };
})();
"""

def newest_content_file():
    files = list(CONTENT_DIR.glob("*.html"))
    if not files:
        return None, 0
...
<body>
  ...
  <div class="main">
    <div id="claude-content">{html}</div>
  </div>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/api/current-mtime":
            _, mtime = newest_content_file()
            self.send_json({"mtime": mtime})
        elif self.path == "/":
            f, _ = newest_content_file()
            if f:
                raw = f.read_text(encoding="utf-8")
                if raw.lstrip().lower().startswith("<!doctype") or raw.lstrip().lower().startswith("<html"):
                    content = raw
                else:
                    content = wrap_fragment(raw)
            else:
                content = wrap_fragment("<p class='subtitle' style='padding:2rem'>Waiting for content...</p>")
    ...
    def do_POST(self):
        if self.path == "/api/event":
            ...
            try:
                ...
                # Clear events when new screen pushed, append otherwise
...
if __name__ == "__main__":
    ...
    print(f"Preview server running on http://localhost:{PORT}", flush=True)
