# Prompt: Review Gate Server Management And Turn-Based Approval Design
...
```text
04b4b42 Fix review gate artifact image URLs
```
...
There may also be a transient local review server from the prior session:
...
Stop only the review server process if needed; do not kill unrelated processes.
...
scripts/playwright_cli_finalize_review.py
...
- The review gate is a human-in-the-loop boundary. Browser clicks create only draft review state; downstream audit steps must require agent-session confirmation and `human-approval.json`.
...
3db5afa Add Playwright CLI human review gate
e1a0a87 Open Playwright review gate from server
04b4b42 Fix review gate artifact image URLs
```
...
Existing smoke/review commands:
...
`scripts/playwright_cli_review_server.py` serves a local carousel at `http://127.0.0.1:8765/`:
...
- exclusive `Accept` / `Needs review` / `Reject` switch on each slide;
- `Accept` selected by default;
- comment input appears when `Needs review` or `Reject` is selected;
- browser writes only:
...
Downstream audit steps should require `human-approval.json`, not `review-draft.json`.
...
- Running `python3 scripts/playwright_cli_review_server.py ... --open` in a normal foreground tool call works while the tool session remains active.
...
```python
subprocess.Popen(
    [sys.executable, "scripts/playwright_cli_review_server.py", "artifacts/playwright-cli-public-page-matrix/latest/manifest.json", "--port", "8765"],
    cwd=repo,
...
python3 scripts/playwright_cli_review_gate.py finalize artifacts/playwright-cli-public-page-matrix/latest/manifest.json
```

The manager should probably:

- write a PID file under the ignored artifact directory, for example `review-server.pid`;
- write logs under the ignored artifact directory, for example `review-server.log`;
...
- Should `review-draft.json` include a session nonce or run id so the agent can prove it is finalizing the same review session the user saw?
...
- Should the review server expose a read-only status endpoint that the agent can poll when the user says `ready`?
- Should the page itself show a copyable "ready payload" that the user pastes back into the agent session?
- Should the manager support multiple simultaneous review gates by choosing ports automatically and writing the chosen URL into a state file?

Suggested first implementation slice:

1. Add `scripts/playwright_cli_review_gate.py` with `start/status/stop/open`.
2. Keep `finalize` in `scripts/playwright_cli_finalize_review.py` for now, or delegate to it from the manager only after the user says `ready`.
...
```bash
git diff --check
python3 -m py_compile scripts/playwright_cli_review_server.py scripts/playwright_cli_finalize_review.py scripts/playwright_cli_review_gate.py
cd mcp-server && ./.venv/bin/pytest -q
python3 scripts/playwright_cli_review_gate.py start artifacts/playwright-cli-public-page-matrix/latest/manifest.json --open
python3 scripts/playwright_cli_review_gate.py status artifacts/playwright-cli-public-page-matrix/latest/manifest.json
curl -fsS -o /dev/null -w '%{http_code}\n' http://127.0.0.1:8765/
curl -fsS -o /tmp/eba-review-check.png -w '%{http_code} %{content_type} %{size_download}\n' http://127.0.0.1:8765/artifact/artifacts/playwright-cli-public-page-matrix/latest/mozilla-careers/viewport.png
python3 scripts/playwright_cli_review_gate.py stop artifacts/playwright-cli-public-page-matrix/latest/manifest.json
```
